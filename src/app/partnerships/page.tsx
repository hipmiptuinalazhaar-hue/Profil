...
export default async function PartnershipsPage({ searchParams }: PartnershipProps) {
  ...
  const subject = encodeURIComponent(isId ? "Usulan kolaborasi strategis — HIPMI PT UIN Al Azhaar" : "Strategic collaboration proposal — HIPMI PT UIN Al Azhaar");
    ...
    <div className="phase-three inner-page phase-nine" lang={locale}>
      ...
      <main id="main-content">
        <section className="inner-hero partnerships-hero">
          <div className="shell inner-hero__grid">
            ...
            <div className="inner-hero__copy">
              <p>{isId ? "Strategic collaboration" : "Strategic collaboration"}</p>
              <h1>{isId ? "Kolaborasi harus menciptakan nilai yang jelas untuk kedua sisi." : "Collaboration should create clear value for both sides."}</h1>
              <span>{isId ? "HIPMI PT membuka ruang kerja sama yang relevan dengan pengembangan pengusaha muda kampus, selama tujuan, peran, manfaat, dan bentuk dukungannya dapat dijelaskan dengan terbuka." : "HIPMI PT welcomes collaboration relevant to developing young campus entrepreneurs when the objective, roles, value, and form of support can be explained clearly."}</span>
            </div>
          </div>
        </section>
        ...
        <section className="collaboration-tracks">
          ...
          <div className="shell collaboration-tracks__grid">
            {collaborationTracks.map((track, index) => (
              <article key={track.title.id}><span>{String(index + 1).padStart(2, "0")}</span><h3>{isId ? track.title.id : track.title.en}</h3><p>{isId ? track.body.id : track.body.en}</p></article>
            ))}
          </div>
        </section>
        ...
        <section className="collaboration-contact">
          <div className="shell collaboration-contact__grid">
            <div><p>{isId ? "Mulai percakapan" : "Start a conversation"}</p><h2>{isId ? "Kirim konteks lengkap sejak pesan pertama." : "Send useful context from the first message."}</h2><span>{isId ? "Tidak ada formulir publik yang menyimpan proposal ke database. Tombol ini membuka email dengan kerangka informasi yang diperlukan." : "There is no public form storing proposals in a database. This button opens email with the information framework we need."}</span></div>
            <a href={`mailto:${siteConfig.contact.email}?subject=${subject}&body=${body}`}><span>{isId ? "Ajukan kolaborasi" : "Propose collaboration"}</span><strong>{siteConfig.contact.email}</strong><i aria-hidden="true">↗</i></a>
          </div>
        </section>
      </main>
